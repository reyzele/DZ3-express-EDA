...
ENGINE.on("index/get", async response => {
  const goods = await ENGINE.emit("goods/get");
  const skills = await ENGINE.emit("skills/get");

  response.reply({ goods, skills });
});
...
ENGINE.on("goods/post", async response => {
  ...
  form.parse(response.data, function(err, fields, files) {
    if (err) {
